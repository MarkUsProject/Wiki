...
> 🗒️ **NOTE:** MarkUs does not check the "last modified time" of the target of a submitted URL. It is possible for students to submit a URL before the deadline, but change the target's contents after the deadline (e.g., by modifying the contents of a Google Doc).
...
> 🗒️ **NOTE:** These tokens can be expired so this is a good way to make results available to students only by request and only for a specific period of time.
...
Although this does not delete the assignment from your dashboard, it does remove it from all the student accounts. Students will neither be able to see nor access deleted assignments.
