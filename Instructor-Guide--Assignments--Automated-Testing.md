...
See the [Using the File Manager](General-Usage.md#using-the-file-manager) section for instructions on uploading and downloading files.
...
Tests are organized into groups which in turn can be associated with [Criteria](Instructor-Guide--Assignments--Marking--Criteria.md). Each test group can be associated with exactly one criterion and the results from the test will [automatically fill in the results](#criterion-auto-complete) when the tests finish.
...
- **Path:** Specify the filename of the file submitted by the student you wish to check with PyTA. Because of this it is advised that you create [required file names](Instructor-Guide--Assignments--Setting-Up.md#required-files) for this assignment. Add another file by clicking the green + under this section.
