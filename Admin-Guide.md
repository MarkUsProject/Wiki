...
### Archiving, removing, and restoring a course

The following actions must be performed as rake tasks from the command line.

#### Archiving

To archive a course to a tar.gz archive file, run the `markus:archive:course` rake task. This task takes a single argument which is the name of the course to archive. For example, in order to archive a course named `myCourse` on the MarkUs instance installed at `/path/to/markus/instance/`:

```sh
cd /path/to/markus/instance
RAILS_ENV=production ./bin/rails markus:archive:course[myCourse]
```

When the rake task finishes, it will print the location of the tar.gz archive file to stdout. You may then copy this file elsewhere for safe-keeping.

#### Removing

To remove a course from a MarkUs instance, run the `markus:remove:course` rake task. This task takes a single argument which is the name of the course to remove. For example, in order to remove a course named `myCourse` on the MarkUs instance installed at `/path/to/markus/instance/`:

```sh
cd /path/to/markus/instance
RAILS_ENV=production ./bin/rails markus:remove:course[myCourse]
```

Note that this cannot be reversed so make sure you archive the course before removing it if you want to keep a record of the data.

#### Restoring

To restore a previously archived course to a MarkUs instance, run the `markus:unarchive:course` rake task. This task takes a single argument which is a path to the tar.gz archive file previously generated by running the `markus:archive:course` rake task (see above).

This task requires that a database be created temporarily in order to migrate and update the data stored in the archive. This means that the database user that MarkUs uses (see `config/database.yml`) must have permission to create databases.

If this user does not have database creation permissions you can pre-create an empty database for this task and specify the connection url to that database with the `TMP_DB_URL` environment variable.

For example, in order to restore a course stored in a file at `/markus-archive/myCourse.tar.gz` on the MarkUs instance installed at `/path/to/markus/instance/`:

```sh
cd /path/to/markus/instance
RAILS_ENV=production ./bin/rails markus:unarchive:course[/markus-archive/myCourse.tar.gz]
```

Note that this task will report any conflicts or errors that occur when uploading the course data. If any errors occur, you will have the option to commit the changes anyway, or roll back the changes at the end.
